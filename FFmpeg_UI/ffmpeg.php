<?php
	...
	$nvidia_gpu = "-hwaccel cuda -hwaccel_output_format cuda -c:v h264_cuvid";
	...
	$ffmpeg_line = "ffmpeg $loglevel -y -i $input_file -map 0:0 $map_0_1 $map_0_2 $map_0_3 $map_0_4 $map_0_5 $map_0_6 $map_0_7 $scale $video_bitrate $video_codec $map_0_1_codec $map_0_1_bitrate $map_0_2_codec $map_0_2_bitrate $map_0_3_codec $map_0_3_bitrate $map_0_4_codec $map_0_4_bitrate $map_0_5_codec $map_0_5_bitrate $map_0_6_codec $map_0_6_bitrate $map_0_7_subtitle $output_file_format.$file_end $format 2> progress.txt";
	...
	echo "</br>Encoding time " . $hours .":". $minutes .":". $seconds . "</br> ...select a new file or press 'Reload'</br>";
	}
	...
	$fps = exec("ffprobe -v error -select_streams v -of default=noprint_wrappers=1:nokey=1 -show_entries stream=r_frame_rate $video_info");
	$fps = round(strstr($fps, '/' , true) / substr (strstr($fps, '/'), 1 ), 2);


	
	
	//$fps = $fps_main / $fps_second;
		
	
	try {
	$info = new SimpleXMLElement(file_get_contents("temp_file"));
	} catch (Exception $e) {
	echo "<div class='alert alert-danger'>ERROR: File contains spaces, use 'Del. Spaces' button.</div>";
	}
	...
	<table  border="0">
	<tr ><th>Video codec</th> <th>Video bitrate</th> <th>Scale</th>  <th>Format</th></tr>
	<td ><select name="video_codec"> 
	...
	<option  value="-b:v 3M -maxrate 3M -bufsize 3M">3.000 kb/s</option>  
	<option value="-b:v 4M -maxrate 4M -bufsize 4M">4.000 kb/s</option>
	<option selected value="-b:v 5M -maxrate 5M -bufsize 5M">5.000 kb/s</option>
	...
	<option  value="-vf scale_vaapi=h=-2:w=1280">720p</option>   
	<option selected value="-vf scale_vaapi=h=-2:w=1920">1080p</option>
	<option value="-vf scale_vaapi=h=-2:w=2048">2K UHD</option>
	<option value="-vf scale_vaapi=h=-2:w=4096">4K UHD</option>
	<option value="-vf scale_vaapi=h=-2:w=7680">8K UHD</option>
	<option value="-vf scale_vaapi=w=-2:h=144">144p</option>
	...
	<!--0:1#############################################################-->
	<tr ><th >Audio Track 0:1</th>  <th>Audio codec</th> <th>Audio bitrate</th></tr>
	<td ><select  name="map_0_1">
	<option value="">nothing</option>
	<option selected value="-map 0:1">0:1</option> <!--////// note 'selected' in this line, this value is preselected in the form-->
	</select></td>        
	<td ><select name="map_0_1_codec">
	<option value="">nothing</option> 
	...
	<td ><select name="map_0_1_bitrate">
	...
	<!--0:3#############################################################################-->
	<tr ><th >Audio Track 0:3</th>  <th>Audio codec</th> <th>Audio bitrate</th></tr>
	...
	<option  value="-map 0:7">0:7 Subtitle</option>
	<option  value="-map 0:8">0:8 Subtitle</option>
	<option  value="-map 0:9">0:9 Subtitle</option>
	<option  value="-map 0:10">0:10 Subtitle</option>
	<option  value="-map 0:11">0:11 Subtitle</option>
	<option  value="-map 0:12">0:12 Subtitle</option>
	<option  value="-map 0:13">0:13 Subtitle</option>
	<option  value="-map 0:14">0:14 Subtitle</option>
	<option  value="-map 0:15">0:15 Subtitle</option>
	<option  value="-map 0:16">0:16 Subtitle</option>
	<option  value="-map 0:17">0:17 Subtitle</option>
	<option  value="-map 0:18">0:18 Subtitle</option>
	<option  value="-map 0:19">0:19 Subtitle</option>
	<option  value="-map 0:20">0:20 Subtitle</option>
	<option  value="-map 0:21">0:21 Subtitle</option>
	<option  value="-map 0:22">0:22 Subtitle</option>
	<option  value="-map 0:23">0:23 Subtitle</option>
	<option  value="-map 0:24">0:24 Subtitle</option>
	<option  value="-map 0:25">0:25 Subtitle</option>
	...
	<!--////////////////////////////////////// show media info using ffprobe + mediainfo-->
	
	<?php    
 	$input_file        = $path.$_POST['file'];
 	$video_bitrate = "mediainfo --Output='Video;%BitRate/String%' '$input_file'";
	$show_video_bitrate = exec("$video_bitrate"); 
	...
	echo "FPS: " . $fps . "</br>";
	echo "Video Bitrate: " . $show_video_bitrate . "</br>";
